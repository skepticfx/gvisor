...
webhook-build: load-jekyll ## Build the webhookimage locally.
	@$(call run,//webhook:webhook,$(WEBHOOK_IMAGE))
.PHONY: webhook-build
...
webhook-update: test/kubernetes/gvisor-injection-admission-webhook.yaml.in
	@WEBHOOK=$(WEBHOOK_IMAGE):$$($(call run,//webhook:webhook,$(WEBHOOK_IMAGE)) | cut -d':' -f2) && \
